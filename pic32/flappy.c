...
{
    lcd_rect(color, x, y, x+w-1, y+h-1);
}

void fill(int color, int x, int y, int w, int h)
{
    lcd_fill(color, x, y, x+w-1, y+h-1);
}
...
void start_game()
{
    ...
    lcd_clear(BLUE);
    lcd_text(&font_lucidasans28, WHITE, BLUE, 10, 10,
        "Flappy Bird");
    lcd_text(&font_lucidasans15, WHITE, BLUE, 10, 180,
        "Press Button to start");

    char score_line[80];
    sprintf(score_line, "High Score: %u", high_score);
    lcd_text(&font_lucidasans15, GREEN, BLUE, 10, 60, score_line);

    /* Draw ground. */
    int tx, ty = 230;
    for (tx = 0; tx <= 300; tx += 20) {
        lcd_fill_triangle(GREEN,  tx,    ty,   tx+9,  ty, tx,    ty+9);
        lcd_fill_triangle(YELLOW, tx+9,  ty+9, tx+9,  ty, tx,    ty+9);
        lcd_fill_triangle(YELLOW, tx+10, ty,   tx+19, ty, tx+10, ty+9);
        lcd_fill_triangle(GREEN,  tx+19, ty+9, tx+19, ty, tx+10, ty+9);
    }
...
void check_collision()
{
    ...
    if (crashed) {
        lcd_text(&font_lucidasans28, RED, BLUE, 10, 50, "Game Over!");

        char score_line[80];
        sprintf(score_line, "Score: %u", score);
        lcd_text(&font_lucidasans28, RED, BLUE, 10, 100, score_line);

        if (score > high_score) {
            high_score = score;
            lcd_text(&font_lucidasans28, RED, BLUE, 10, 150, "NEW HIGH!");
            save_score();
        }
...
int main()
{
    ...
    printf("Play Flappy Bird: use buttons on LCD board.\n");
    lcd_init(0, BLUE, &xsize, &ysize);
